...
def get_link_data_from_story(file_path: str) -> List[Dict]:
    ...
    data = [dict(
        ...
        source_media_id=story['media_id'],
...
@task
def write_link_file(link_csv_file_path: str, link_ndjson_file_path: str, links_per_story: List[List[Dict]]) -> None:
    logger = prefect.context.get("logger")
    field_names = ["link_id", "source_stories_id", "publication_date", "sentence", "source_url", "source_media_id", "source_domain", "link_text", "target_url", "target_domain", "source_country", "week_number", "source_nyt_themes", "source_story_is_politics", "source_story_is_health", "source_story_is_economics", "source_story_sentence_count", "source_story_is_sports", "is_self_link"]
    csv_writer = csv.DictWriter(io.open(link_csv_file_path, 'w', encoding='utf8'), field_names, extrasaction='ignore')
    csv_writer.writeheader()
    ndjson_file = io.open(link_ndjson_file_path, 'w', encoding='utf8')
...
@task
def get_story_data(file_path: str) -> Dict:
    story = json.load(open(file_path))
    theme_data = analysis.nyt_theme(story['story_tags'])
    # parse out the links we care about
    data = dict(
        stories_id=story['stories_id'],
        publication_date=story['publish_date'][0:19],
        url=story['url'],
        media_id=story['media_id'],
        domain=get_canonical_mediacloud_domain(story['url']),
        is_politics=theme_data['is_politics'],
        is_health=theme_data['is_health'],
        is_economics=theme_data['is_economics'],
        is_sports=theme_data['is_sports'],
        country=analysis.country_alpha3(story['media_id']),
        week_index=analysis.study_week_index(story['publish_date']),
        nyt_themes=theme_data['nyt_theme_tag_ids'],
    )
    return data


@task
def write_story_csv(link_csv_output_file: str, stories_list: List[Dict]) -> None:
    logger = prefect.context.get("logger")
    field_names = ["stories_id", "publication_date", "url", "media_id", "domain", "is_politics", "is_health", "is_economics", "is_sports", "country", "week_index", "nyt_themes"]
    csv_writer = csv.DictWriter(io.open(link_csv_output_file, 'w', encoding='utf8'), field_names, extrasaction='ignore')
    csv_writer.writeheader()
    logger.info("    need to write {} stories".format(len(stories_list)))
    # and export the links
