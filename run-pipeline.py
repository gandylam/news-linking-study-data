...
if __name__ == '__main__':
    ...
    with Flow("link-extraction") as flow:
        ...
        story_csv_output_file = Parameter("story_csv_output_file", default="story-data.csv")
        tasks.write_story_csv(story_csv_output_file, story_data)
    ...
    for media_dir in sorted(media_dirs):
        ...
        logger.info("-----------------------------------------------------------------------------------------------")
        if not os.path.exists(story_csv_file) and not os.path.exists(link_csv_file) and not os.path.exists(link_ndjson_file):
            logger.warning("Starting {}: files already exist".format(media_id))
            flow.run(parameters={
                'input_dir': media_dir,
            ...
            logger.warning("Skippping {}: files already exist".format(media_id))
